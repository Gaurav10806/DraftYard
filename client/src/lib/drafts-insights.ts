...
export function stageToProgress(stage: string | undefined): number {
  if (!stage) return 35; // Default for undefined/empty
...
export function teamSizeVsStage(list: Draft[] = drafts) {
  ...
  const stages = Array.from(new Set(list.map((d) => d.currentStage || d.stageDied || "Unknown")));
  return stages.map((stage) => {
    const row: Record<string, string | number> = { stage };
    for (const size of sizes) row[size] = list.filter((d) => (d.currentStage || d.stageDied) === stage && d.teamSize === size).length;
    return row;
  });
}
