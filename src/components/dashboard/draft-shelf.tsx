...
export function DraftShelf() {
  ...
  return (
    <div className="min-w-0 rounded-2xl border border-border/70 bg-card p-6 shadow-sm transition-shadow duration-300 hover:shadow-md">
        ...
        <div ref={scroller} className="scrollbar-none flex snap-x snap-mandatory gap-4 overflow-x-auto scroll-smooth pb-2">
          {items.map((d, i) => {
            ...
            return (
              <div
                key={d.projectName}
                className="group/card relative snap-start shrink-0 w-64 rounded-2xl border border-border/70 bg-background p-4 transition-all duration-300 hover:-translate-y-1 hover:shadow-lg hover:border-primary/40"
              >
                <div className="flex items-center justify-between">
                  {pinned ? (
                ...
                <div className="pointer-events-none absolute inset-x-3 bottom-3 flex gap-2 opacity-0 translate-y-1 transition-all duration-200 group-hover/card:opacity-100 group-hover/card:translate-y-0 group-hover/card:pointer-events-auto">
                  <button className="flex-1 rounded-lg bg-primary px-2 py-1.5 text-[11px] font-semibold text-primary-foreground shadow-sm hover:brightness-110">Open</button>
                  <button className="flex-1 rounded-lg border border-border bg-background px-2 py-1.5 text-[11px] font-semibold hover:border-primary/50">Revive</button>
                </div>
              </div>
            );
